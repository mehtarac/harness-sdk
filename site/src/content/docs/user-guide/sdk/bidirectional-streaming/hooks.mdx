...
`BidiAgent` uses the same batch and per-tool hooks as `Agent`.
`BeforeToolsEvent` and `AfterToolsEvent` run once for each provider-defined
group. `BeforeToolCallEvent` and `AfterToolCallEvent` run for each tool that
the group executes.
...
def validate_group(event: BeforeToolsEvent) -> None:
    if len(event.message["content"]) > 5:
        event.cancel = "Too many tools requested in one group"


def log_group(event: AfterToolsEvent) -> None:
    print(f"Completed {len(event.message['content'])} tool results")


agent = BidiAgent(tools=[inspect_agent])
agent.add_hook(validate_group)
agent.add_hook(log_tool_call)
agent.add_hook(retry_failed_tool)
agent.add_hook(log_group)
```
...
| `BeforeToolsEvent` | Before executing a complete provider-defined tool group |
| `AfterToolsEvent` | After the group execution phase; reverse callback ordering |
| `BeforeToolCallEvent` | Before executing one tool |
| `AfterToolCallEvent` | After one tool executes; reverse callback ordering |
