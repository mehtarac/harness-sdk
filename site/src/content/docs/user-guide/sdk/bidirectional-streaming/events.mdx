...
Tool events separate visibility from execution authorization. A provider first
streams each tool request, then emits one complete provider-defined group.

#### ToolUseStreamEvent

Emitted while the model provider exposes a tool request. This event does not
authorize execution.
...
Emitted once when the model provider confirms that a complete tool group is
ready for execution.
...
- `message`: One assistant message containing only `toolUse` blocks
- `tool_uses`: The complete tool requests in provider order

The agent executes tools only after this event. It records one assistant
tool-use message and one ordered user tool-result message for the group.

Group boundaries follow the model provider's protocol. Gemini Live groups its
atomic tool-call list, OpenAI uses completed calls from `response.done`, and
Nova Sonic emits one group for each matching TOOL `contentEnd`.
