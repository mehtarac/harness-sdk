...
async def test_send_all_content_types(nova_model, mock_stream):
    ...
    nova_model._pending_tool_use_ids.add("tool-123")
    ...
    nova_model._pending_tool_use_ids.add("tool-456")
...
async def test_event_conversion(nova_model):
    ...
    response_state.tool_content_id = "tool-content"
    nova_event = {
        "toolUse": {
    ...
    complete = nova_model._convert_nova_event(
        {"contentEnd": {"type": "TOOL", "contentId": "tool-content", "stopReason": "TOOL_USE"}},
        response_state,
    )
    assert isinstance(complete[0], BidiToolUsesCompleteEvent)
    assert nova_model._pending_tool_use_ids == {"tool-123"}
...
async def test_tool_result_single_content_unwrapped(nova_model, mock_stream):
    ...
    assert not nova_model._pending_tool_use_ids
...
async def test_tool_result_empty_content(nova_model, mock_stream):
    ...
    tool_result: ToolResult = {"toolUseId": "tool-789", "status": "success", "content": []}

    nova_model._pending_tool_use_ids.add("tool-789")
    await nova_model.send_tool_results(_tool_result_message(tool_result))
...
@pytest.mark.asyncio
async def test_tool_result_must_match_pending_tool_call(nova_model):
    await nova_model.start()
    ...
    with pytest.raises(ValueError, match=r"result does not match a pending Nova tool call"):
        await nova_model.send_tool_results(_tool_result_message(tool_result))
...
async def test_tool_result_unsupported_content_type(nova_model):
    ...
    nova_model._pending_tool_use_ids.add("tool-999")
    ...
    nova_model._pending_tool_use_ids.add("tool-888")
    with pytest.raises(ValueError, match=r"content type not supported by Nova Sonic"):
    ...
    nova_model._pending_tool_use_ids.add("tool-777")
    with pytest.raises(ValueError, match=r"content type not supported by Nova Sonic"):
...
def test_user_transcript_closes_response_after_tool_use(nova_model, assistant_transcript, partial_audio):
    state = _ResponseState(
        ...
        tool_content_id="tool",
        tool_use={"toolUseId": "call", "name": "time_tool", "input": {}},
    )
    native_events = [
    ...
    exp_events = [
        BidiToolUsesCompleteEvent(
            {
                "role": "assistant",
                "content": [{"toolUse": {"toolUseId": "call", "name": "time_tool", "input": {}}}],
            }
        ),
        *(
            [
