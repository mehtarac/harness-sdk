...
def _validate_tool_use_message(message: Message) -> None:
    """Validate an assistant message containing one complete tool-use group."""
    if not isinstance(message, dict) or message.get("role") != "assistant":
        raise ValueError("tool-use message must have role 'assistant'")

    content = message.get("content")
    if not isinstance(content, list) or not content:
        raise ValueError("tool-use message content must not be empty")
